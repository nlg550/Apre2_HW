...
def create_cnn(img_shape, regularization=False, initialize=''):
    ...
    if not regularization:
        model.add(tf.keras.layers.Dense(1024, activation=tf.nn.relu, use_bias=True))
        model.add(tf.keras.layers.Dense(512, activation=tf.nn.relu, use_bias=True))
    else:
        initializer = None
        if initialize == 'ones':
        ...
        model.add(tf.keras.layers.Dense(1024, activation=tf.nn.relu, kernel_initializer=initializer, use_bias=True,
                                        kernel_regularizer=tf.keras.regularizers.l2(0.001)))
        model.add(tf.keras.layers.Dropout(rate=0.3))
        model.add(tf.keras.layers.Dense(512, activation=tf.nn.relu, kernel_initializer=initializer, use_bias=True,
...
if __name__ == "__main__":
    ...
    model, history = train(model, train_gen, validation_gen, batch_size, n_epoch=18)
